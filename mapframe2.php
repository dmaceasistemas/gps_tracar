...
deleteMarkers();
 downloadUrl("phpsqlajax_genxml3.php?UID=<?php echo $GPSUSERID;?>", function(data) {
      ...
      if ((dt_ver==<?php echo $vehi;?>)||(dt_id==<?php echo $vehiculo;?>)) 
...
// AÑADE UN MARCADOR CUANDO SE HACE CLIC EN EL MAPA
		 ...
		 } // FIN DEL CICLO PARA MOSTRAR TIPO DE VEHICULO		 
...
function GetAddressByGoogle(t, lat, lng) {
  ...
  <body>
  <form name="frmset" method="post" action="mapframe2.php" >
<div id="disSecond" style="position:absolute; width:365px; height:25px; box-shadow:#888 1px 1px 0px; -moz-box-shadow:#888 1px 1px 0px; background-color:White; margin-left:1px; font-size:15px; margin-top:-9px; line-height:100%; z-index:999;">
...
<div style="position:absolute; width:305px;height:25px; box-shadow:#888 1px 1px 0px;background-color:White;margin-left:1px; font-size:12px;margin-top:20px; line-height:100%; z-index:999;">
   <tr>
        <td align="left">Tipo de Vehiculo</td>
		<select name="vehiculo" id="vehiculo" value="<?php echo @$_POST['vehiculo']?>">

        <?php 
     
        
        if(isset($_POST["vehiculo"]))
        	$esta_vehi=$_POST["vehiculo"];
        $sql2=mysql_query("SELECT * FROM device_type WHERE dt_id=".$esta_vehi."");     
     ...
     /*   
         echo '<option>- Tipo del vehiculo -</option>';   
         $sql=mysql_query("SELECT * FROM device_type WHERE dt_status='1'");
           while($row=mysql_fetch_array($sql))
            {
        	echo '<option value="'.$row['dt_id'].'">'.$row['dt_name'].'</option>';
	...
	*/
        ...
        <input type="submit" value="Ver" id="ver" name="ver" autofocus >
   
      </tr>
  </div>
  <div style="width:210px; height:520px; overflow:scroll; margin:45px 2px 2px 0px; float:left;">
